 ...
 *
 * <p>lsp4ij writes LSP requests to {@link #getOutputStream()}, which this
 * provider reads, translates to the legacy {@code lsp.handle} protocol, and
 * forwards to the running Dart Analysis Server. Responses from the server
 * containing {@code lspMessage} or {@code lspResponse} payloads are written
 * back to {@link #getInputStream()} for lsp4ij to consume.</p>
 */
class DartVirtualStreamConnectionProvider implements StreamConnectionProvider {
   ...
   */
  private static JsonObject extractLspPayload(JsonObject jsonObject) {
